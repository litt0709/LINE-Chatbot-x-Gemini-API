...
| :--- | :--- |
| `index.js` | Điểm truy cập Webhook cho LINE & Telegram. Chứa `masterScheduler` xử lý cron (dọn dẹp, bản tin). Quản lý Regex parsing tags (`<PROFILE>`, `<FACT>`, `<TOPIC>`, `<REACT>`, `<Task>`). |
...
| `utils/deepseek.js` | API tương tác với mô hình `deepseek-v4-flash`. Quản lý System Prompt siêu tối giản, cơ chế ép xuất XML tags, và gen smart search query. |
...
| `utils/search.js` | Nhận diện ngữ cảnh Web/Search từ nội dung chat. Tự động scrape các URLs, hoặc gọi APIs tìm kiếm dựa theo phân nhóm regex (NEWS, FINANCE, DEV, SOCIAL). |
...
- B5: Gọi `search.js` kiểm tra Regex xem nội dung có cần search thông tin thời sự mới nhất hay không.
- B6: Tổng hợp Prompt gửi sang DeepSeek (`deepseek-v4-flash`).
- B7: Tại `index.js`, dùng Regex linh hoạt (dựa trên `getAttr`) bóc tách `<PROFILE>`, `<FACT>`, `<REACT>`, `<TOPIC>`.
- B8: Trả về câu trả lời đã dọn sạch XML tag và lưu tin nhắn nguyên bản vào RTDB.
...
3. **Firestore**: Chỉ ghi log kiểm toán dài hạn (Audit Logs), Profiles cố định, cấu hình giới hạn (Quota). Ít bị gọi nhất.
